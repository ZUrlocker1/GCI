...
| Piece HP | N/A | Each piece has hit points; damage shown by chipped sprite |
...
Black's chess move happens at the *end* of the fleet's lateral phase each turn. The engine picks the best legal chess move; the selected piece teleports/slides to its new square (with a brief animation). This means black pieces can suddenly relocate out of formation — which creates unpredictability.
...
- Galaxian Escort/Flagship: bright colored pixel-art ships, dive accompanied by a swooping pitch-slide sound effect.
...
**Damage is shown directly on the sprite — no HP bars.** Each hit chips away visible pixels from the piece, so it literally looks battle-damaged. Higher-value pieces are larger and more heavily built, so the same number of hits produces proportionally less visual destruction:

| Piece | Max HP | Visual damage per hit (% of sprite chipped) |
|---|---|---|
| Pawn | 2 | ~50% — one hit cracks it badly; second destroys |
| Knight | 6 | ~16% per hit — noticeably eroding by hit 3 |
| Bishop | 6 | ~16% per hit — diagonal gouges across the mitre |
| Rook | 8 | ~12% per hit — chunks break off the castle towers |
| Queen | 12 | ~8% per hit — corona dims, crown cracks gradually |
| King | 16 | ~6% per hit — slow, dramatic deterioration |

**Implementation:** Each piece type has a sprite sheet with **damage frames** (one frame per HP tier, or a small set of pre-rendered chipped variants). The renderer picks the frame matching the current HP bracket. No overlay bars are drawn — the piece sprite itself tells the story. Chip patterns are asymmetric and irregular so each damaged piece looks unique rather than uniformly eroded.
...
- Single-shot laser (must wait for the previous shot to exit the screen or hit something before firing again).
...
- Black chess moves: **2 per turn** — two pieces relocate each turn, making the formation unpredictable
...
- Black chess moves: **3 per turn** — the board reshuffles dramatically every 5 seconds
...
The visual style blends **classic 80s arcade pixel art** with the **Atari Recharged aesthetic** — the modern Atari reboot series (Asteroids Recharged, Breakout Recharged, etc.) that pairs deep black backgrounds with electric neon outlines, vector-like glow bloom, and high-contrast color pops. The result looks retro in spirit but vivid and sharp on modern Retina displays.

**Core principles:**
- Pure black (`#000000`) backgrounds — no grey, no gradients behind gameplay elements
- All sprites and geometry are outlined in bright neon colors with a soft bloom/glow halo (rendered via a blur-and-add shader pass in SpriteKit)
- Limited neon palette: electric blue, hot cyan, acid green, magenta, orange, white — never more than 3–4 colors on a single sprite
- Pixel art construction (on a strict grid) but with smooth glow effects layered on top — the "Recharged" trick that makes pixel art feel alive
- Thin, luminous lines for the board grid — more vector/Tron than painted wood
- Everything glows. Lasers glow. Pieces glow. The board grid glows faintly. Explosions bloom outward.

The tone is neon space-opera: like playing chess inside a laser light show at a 1983 arcade.

---

### 12.2 Piece Sprite Design

Chess pieces use **classic 1980s pixel-art silhouettes** — the kind found in Sargon II/III on the Apple II and early IBM PC chess programs. The shapes are immediately recognizable as traditional chess pieces: the pawn's rounded head, the knight's horse profile, the bishop's mitre, the rook's battlements, the queen's tall crown, the king's cross-topped crown. These are not spaceships. They are chess pieces, rendered in the pixelated style of their era and then given a neon Recharged glow treatment.

**Reference aesthetic:** Sargon III (Apple II, 1983), Chessmaster 2000 (PC, 1986) — chunky, clearly readable 2D top-or-side silhouettes, each piece distinct at a glance even at 16–20 px.

#### Palette & Glow

Both sides share the same silhouette shapes. Color and glow distinguish them:

| Side | Fill color | Outline / glow | Effect |
|---|---|---|---|
| **White (player)** | Pale luminous white (`#E8F4FF`) | Soft cyan-white bloom (`#00DFFF`) | Steady glow, clean |
| **Black (enemy)** | Deep charcoal (`#1A0A2E`) with lit edges | Magenta/red bloom (`#FF2060`) | Slightly more intense pulse |

The two palettes are consistent across all six piece types on each side. A player can identify side instantly without reading the shape.

#### Sprite Sizes & Shapes

| Piece | Silhouette | Size |
|---|---|---|
| **Pawn** | Round head, short neck, flared base — classic pawn profile | 16×16 px |
| **Knight** | Horse head facing right (player side) or left (enemy side), mane detail, blocky neck | 20×20 px |
| **Bishop** | Tall mitre with center notch, narrow collar, flared base | 20×20 px |
| **Rook** | Squat tower with 3-merlon battlement top, wide base | 20×20 px |
| **Queen** | Tallest piece, pointed crown with 5 points, narrow waist | 24×24 px |
| **King** | Slightly shorter than queen, wide cross finial on top, broad base | 28×28 px |

Sprites are drawn on a strict pixel grid (no sub-pixel anti-aliasing). `filteringMode = .nearest` in SpriteKit preserves crisp edges on all display sizes. The Recharged glow is a bloom halo rendered by the parent `SKEffectNode` — it is not baked into the sprite sheet, keeping the pixel art clean underneath.

Each piece has **4 animation frames**: idle float (gentle bob of 2–3 px), move (brief brighten flash), damaged (chips appear — see below), and destruction (explosion sequence, 8 frames).

Pieces display damage **directly on the sprite** — no HP bar overlay. Each damage tier is a pre-rendered chipped variant of the base sprite; the piece looks like it is physically being blasted apart:

- **Full HP:** clean sprite, bright glow intact
- **~75% HP:** first chip notch — a corner or edge pixel cluster missing
- **~50% HP:** visibly cracked; smoke particles begin trailing off the piece
- **~25% HP:** heavily fragmented; sprite flickers, flames/sparks visible in the gaps
- **0 HP:** 8-frame explosion sequence, then gone

Higher-value pieces are larger (up to 28×28 px for the King vs 16×16 px for a Pawn), so the same absolute pixel damage looks proportionally smaller — a Queen that has taken four hits still looks imposing, while a Pawn that has taken one hit looks nearly destroyed.
...
The scene has **4 parallax layers**, back to front:
...
- **Title screen:** "GALACTIC CHESS INVADERS" in large pixel font, animated with a starfield behind it. A looping demo plays in the background (attract mode) — showing pieces moving and being shot, cycling after ~20 seconds.
- **High score entry:** classic 3-letter initial entry using up/down arrows per character.
...
- **Game over screen:** "GAME OVER" in red, score displayed, high score comparison, prompt to continue or return to menu.

### 12.3 Audio Design

All audio is 8-bit / chiptune style — square waves, triangle waves, and noise-channel percussion. No sampled audio. Generated or authored in a tool like **FamiTracker**, **BeepBox**, **PICO-8**, or **sfxr/LabChirp** for one-shot effects, then exported as `.wav`/`.caf` files bundled in the app.

#### Soundtrack

The background music is a synth chiptune track that runs continuously during gameplay. It is **not** a static loop — it responds to game state:

| Game state | Music behavior |
|---|---|
| Normal play | Main theme loop, moderate tempo, minor-key arpeggios |
| Fleet advancing (rank 4 or below) | Tempo increases, bass line becomes more prominent |
| Last 5 black pieces remaining | Drums drop out, melody plays sparse and tense |
| Raider Scout on screen | High-pitched counter-melody layer added |
| Flagship dive | Music briefly drops to a single ominous bass pulse |
| Player in check | Dissonant stab chord repeats every 2 beats until resolved |
| Level clear | Victory fanfare jingle (3–4 seconds), then silence before next level |
| Game over | Descending chromatic death riff |

The classic Space Invaders "heartbeat" — the four-note descending bass that speeds up as the fleet shrinks — should be layered under the main theme and accelerate as black pieces are eliminated.
...
- SFX volume (separate)
- Toggle: "Invader heartbeat" on/off (some players find it anxiety-inducing after long sessions)
...
- **Duration:** indefinite — lasts until hit or level ends.
...
- Icon is a ~16×16 px pixel-art glyph inside a glowing diamond outline.
...
- Initials entry screen appears: 8 characters, classic arcade-style selector.
...
- Score and initials are saved to local storage (`UserDefaults`).
- High score table shows top 10 entries: rank, initials, score, level reached.
...
- **Fleet movement** runs continuously via `SKAction.repeatForever` on a fleet parent node — all piece nodes are children, so they shift together. Pieces can move beyond the logical board boundary on screen; chess legality is always checked against their snapped logical square, not their visual position.
...
### 19.3 Sound Effects — jsfxr (tool) + AVFoundation (built-in)

**SFX Generation Tool:** [jsfxr](https://sfxr.me) — browser-based, free, no install.
**Playback:** `AVFoundation` — ships with macOS and iOS, no dependency needed.

**Workflow:**
1. Generate all arcade SFX in jsfxr (laser, explosion, powerup, hit, etc.) — export as `.wav`
2. Convert to `.caf` (Core Audio Format, linear PCM) using the `afconvert` tool that ships with Xcode:
   ```
   afconvert -f caff -d LEI16 laser.wav laser.caf
   ```
   `.caf` with linear PCM has the lowest decode latency on Apple hardware — critical for arcade SFX that must fire on exact frames.
3. Preload all `.caf` files into `AVAudioPlayer` instances at scene load time — zero I/O during gameplay.
4. For polyphonic sounds (rapid laser fire, multiple simultaneous explosions), maintain a pool of 3–4 `AVAudioPlayer` instances per sound and round-robin between them.

**Alternative SFX tools:**
- [ChipTone](https://sfbgames.itch.io/chiptone) — more powerful than jsfxr, free download, excellent for richer arcade sounds
- [jfxr](https://jfxr.frozenfractal.com) — sfxr variant with more parameters

No third-party Swift audio library is needed for SFX. `AVFoundation` handles everything required.

---

### 19.4 Music — FamiStudio (tool) + AVFoundation (built-in)

**Composition Tool:** [FamiStudio](https://famistudio.org) — free, cross-platform NES/chiptune editor.
**Playback:** `AVFoundation` — built-in, no dependency.

**Workflow:**
1. Compose all music tracks in FamiStudio (main theme, heartbeat layer, level clear fanfare, game over riff, title music)
2. Export each track as `.wav` at 44.1kHz stereo
3. Convert to `.m4a` (AAC) for efficient streaming:
   ```
   afconvert -f m4af -d aac music.wav music.m4a
   ```
4. Play with `AVAudioPlayer` in a loop. Use `AVAudioPlayer.numberOfLoops = -1` for seamless infinite looping.
5. The heartbeat layer is a separate `.m4a` file whose `rate` property is adjusted dynamically as pieces are eliminated:
   ```swift
   heartbeatPlayer.rate = 1.0 + (1.0 - Float(remainingPieces) / 16.0) * 1.5
   ```

**Alternative composition tool:** [BeepBox](https://www.beepbox.co) — browser-based, excellent for quick chiptune sketches, exports WAV/MP3. Good for early placeholder music before final tracks are composed.

**If tracker file format is preferred:** Use `OpenMPTSwift` (`https://github.com/lukasz-pomianek/OpenMPTSwift`) to play `.xm` or `.mod` files directly — much smaller file sizes (kilobytes vs megabytes) at the cost of an extra dependency. Evaluate after placeholder music is working.
...
| **FamiStudio** | External tool (not a code dep) | famistudio.org | Compose chiptune music tracks |
| **BeepBox** | External tool (not a code dep) | beepbox.co | Placeholder/alternative music composition |
...
### 19.1 Never Block the Main Thread
...
### 19.2 Object Pooling for Frequently Created Nodes
...
### 19.3 Texture Atlases — One Draw Call Per Atlas
...
### 19.4 Keep the Node Tree Shallow and Small
...
### 19.5 Use SKAction for All Animations
...
### 19.7 Preload All Audio at Startup
...
### 19.8 Delta-Time Based Movement
...
### 19.9 The Bloom Shader — Use Once, Apply Everywhere
...
PROMOTE  Multi-shot activated (cap: 3 lasers)
...
- `PieceNode.swift` — simple colored rectangles at correct sizes (16–28px), correct glow colors (white=cyan, black=magenta); damage frames stubbed as progressively smaller/dimmer rectangles (will be replaced by chipped pixel-art sprites in Phase 2.2)
...
**Goal:** Replace placeholder shapes with final neon pixel-art sprites and apply the full Recharged aesthetic. No new gameplay — purely visual upgrade. Done once here, never revisited.

**Build:**
- Final pixel-art sprite for all 12 piece types — correct silhouettes per §12.2
- `texture.filteringMode = .nearest` on every texture — crisp on Retina
...
- Basic parallax starfield — 2 tiling texture layers scrolling downward (no individual star sprites)
...
- Space Invaders-style heartbeat layer — separate audio track that speeds up as black pieces are eliminated
...
- Heartbeat tempo increases noticeably as fleet thins — verify at 16, 8, 4, 1 pieces remaining
...
- **Pass criteria:** a full playthrough with music feels like a complete arcade experience; heartbeat escalation is felt in the gut
...
**Build:**
- Diagonal invader shots (Level 3+) — 45° projectiles, purple glow, 160 px/s
...
- Pawn promotion triple event — auto-queen swap, targeting beam destroys nearest black piece, multi-shot activated (cap 3 lasers, resets next level)
...
- Multi-shot: 3 simultaneous lasers active; resets to 2 at next level start
...
- Full piece sprite art refinement — final pixel-art detail pass if needed
...
- High score entry (8-character initials, top 10)
...
- Regression test all Phase 0–6 cases after balance changes
...
### 19.1 macOS (Keyboard + Mouse / Trackpad)
...
## 20. Edge Cases & Rules Clarifications
...
### 20.2 Game Over Conditions — Complete List
...
| All black pieces destroyed | Victory — level clear |
...
### 20.5 Piece Logical Position During Fleet Sweep

Black pieces have two positions at all times:
- **Visual position:** where the sprite is drawn on screen, continuously moving left/right with the fleet parent node.
- **Logical position:** the chess square (a1–h8) used for all move generation, collision zone calculation, and projectile targeting.

The logical position updates **only** when a chess move is executed. It does not change during lateral fleet movement. When the player shoots a black piece, the hit is detected against the visual position (the sprite's actual screen location). When the chess engine evaluates moves, it uses logical positions only.

This means a black piece can visually be in column B on screen while logically still occupying column E — which is intentional and creates the arcade feel of a moving fleet that is also playing chess.
...
**Decision:** When the timer expires the computer makes a **reasonable chess move** for white using the same 1-2 ply engine that drives black. This is fairer than random — it won't throw away pieces — but it won't be inspired either. "AUTO" flashes orange above the moved piece for 0.75 seconds.
...
1. Pawn instantly becomes a Queen (full 8 HP, Queen sprite with flash animation + ascending arpeggio).
...
3. The player's spaceship is **upgraded to multi-shot** for the remainder of the level: up to 3 simultaneous lasers on screen instead of 2. A "MULTI-SHOT" banner briefly sweeps across the bottom of the screen.

This makes promoting a pawn feel like an achievement worth engineering. The multi-shot upgrade resets at the start of each new level.
...
**Rule:** Black King is a full member of the fleet parent node and participates in all lateral sweeps. It moves to a new chess square when the engine selects a king move (rare). The King is the highest-HP piece (10 HP) so shooting it down requires sustained fire or a clear lane — it won't die from a stray shot.

---

### 15.8 Spaceship Firing Rate

**Decision:** Up to **2 simultaneous lasers** on screen under normal conditions. This becomes **3** after a pawn promotion power-up (see §15.5), resetting each level.

...
- **Prompts for the controls you have not used** — Three beats without firing raises "PRESS SPACE TO FIRE!" under the hint. Ten seconds of firing without steering raises "USE ARROWS TO MOVE!". Both go the instant the control is used.
...
- **All four prompts re-arm at each level** — A run is long and a wave break is a natural place to forget. They cost nothing to a player who remembers: the fire prompt waits three beats, and by Level 2 anyone still playing is firing within one. All four are independent of the Chess Hints switch — someone who turned hints off can still be the one who has not found the trigger.
